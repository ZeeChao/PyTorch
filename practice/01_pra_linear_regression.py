...
# indices 形状 [100]
# TODO: 用 x.shape[0] 获取样本数 n_samples，用 torch.randperm(n_samples) 生成打乱的索引 indices
n_samples = ...
indices = ...

# 按 70% / 20% / 10% 计算各集合的样本数量
# TODO: 用 int(n_samples * 比例) 计算 n_train（70%）和 n_valid（20%）
n_train = ...
n_valid = ...

# 对打乱后的索引切片，得到三组互不重叠的索引
# train_idx 形状 [70]、valid_idx 形状 [20]、test_idx 形状 [10]
# TODO: 对 indices 切片：train_idx 取前 n_train 个，valid_idx 取接下来 n_valid 个，test_idx 取剩余部分
train_idx = ...
valid_idx = ...
test_idx = ...

# 用索引取出三个集合的数据
# x_train、y_train 形状均 [70, 1]；x_valid、y_valid 形状均 [20, 1]；x_test、y_test 形状均 [10, 1]
# TODO: 用 x[索引]、y[索引] 取出 x_train/y_train、x_valid/y_valid、x_test/y_test
x_train, y_train = ...
x_valid, y_valid = ...
x_test, y_test = ...
...
# w 为标量参数，形状 [1]
# TODO: 用 torch.randn 初始化 w（形状为 1），开启梯度追踪，并指定 device=device
w = ...
# b 为标量参数，形状 [1]
# TODO: 用 torch.zeros 初始化 b（形状为 1），开启梯度追踪，并指定 device=device
b = ...
...
# 返回的 loss 为标量，形状 []
# TODO: 返回 (y_pred - y_true) 的平方的均值（提示：** 2 和 .mean()）
def mse_loss(y_pred, y_true):
    ...

# Step 4.3：优化器

# 本关的求解方法是手写梯度下降：按"参数 -= 学习率 * 梯度"直接更新 w、b，
...
# 返回的 MAE 为标量，形状 []
# TODO: 返回 (y_pred - y_true) 的绝对值的均值（提示：.abs() 和 .mean()）
def mae_metric(y_pred, y_true):
    ...

# 评估指标二：决定系数（R²），衡量模型解释数据变异的能力，越接近 1 越好
# 公式：R² = 1 - 残差平方和 / 总平方和
# 返回的 R² 为标量，形状 []（ss_res、ss_tot 也都是标量，形状 []）
# 隐式广播：y_true.mean() 是标量（形状 []），
# 与 y_true 相减时会广播成与 y_true 相同的形状，再逐元素相减
# TODO: 先算残差平方和 ss_res = ((y_true - y_pred) ** 2).sum()，
#       再算总平方和 ss_tot = ((y_true - y_true.mean()) ** 2).sum()，
#       最后返回 1 - ss_res / ss_tot
...
for epoch in range(epochs):
    ...
    # TODO: 把 loss_train_sum 初始化为 0.0
    loss_train_sum = ...
    # n_train_samples 为本 epoch 已处理的训练样本总数
    # TODO: 把 n_train_samples 初始化为 0
    n_train_samples = ...
    # 整个训练集的各指标变量，先置初值，epoch 末尾统一计算
    # TODO: 把 loss_train、mae_train、r2_train 均初始化为 0.0
    loss_train = ...
    mae_train = ...
    r2_train = ...
    ...
    # TODO: 把 y_pred_tensor_list、y_true_tensor_list 初始化为空列表
    y_pred_tensor_list = ...
    y_true_tensor_list = ...
    ...
    # TODO: 把 x_train、y_train 迁移到 device，赋值给新的 batch 变量
    #       （如 x_batch_train = x_train.to(device)）
    x_batch_train = ...
    y_batch_train = ...
    ...
    # TODO: 判断 w.grad 是否存在，若存在则对 w 和 b 的梯度调用 .zero_()
    ...
    ...
    # TODO: 计算 y_batch_pred = w * x_batch_train + b
    y_batch_pred = ...
    ...
    # loss_batch 为标量，形状 []
    # TODO: 调用 mse_loss，传入 y_batch_pred 和 y_batch_train
    loss_batch = ...
    ...
    # TODO: 在 torch.no_grad() 上下文中，用梯度下降公式更新 w 和 b
    # 提示：w -= learning_rate * w.grad
    # 更新后 w、b 形状仍为 [1]
    with torch.no_grad():
        ...
    ...
    # TODO: 用 x_batch_train.shape[0] 得到 batch_size，
    #       累计 loss_train_sum += loss_batch.item() * batch_size，
    #       累计 n_train_samples += batch_size
    batch_size = ...
    ...
    ...
    ...
    # TODO: 向 y_pred_tensor_list 追加 y_batch_pred.detach()，向 y_true_tensor_list 追加 y_batch_train
    ...
    ...

    # Step 6.9：计算整个训练集的 loss 和各指标（所有 batch 处理完后）
    # loss_train = loss 总和 / 总样本数，与 mse_loss 的 mean 口径一致
    # TODO: 计算 loss_train = loss_train_sum / n_train_samples
    loss_train = ...
    ...
    with torch.no_grad():
        ...
        # y_pred_tensor 形状 [70, 1]、y_true_tensor 形状 [70, 1]
        # TODO: 用 torch.cat(列表, dim=0) 分别拼接出 y_pred_tensor 和 y_true_tensor
        y_pred_tensor = ...
        y_true_tensor = ...

        # MAE：平均偏差，单位与 y 相同
        # mae_train 为标量，形状 []
        # TODO: 调用 mae_metric，传入 y_pred_tensor 和 y_true_tensor
        mae_train = ...

        # R²：越接近 1，模型解释能力越强
        # r2_train 为标量，形状 []
        # TODO: 调用 r2_score，传入 y_pred_tensor 和 y_true_tensor
        r2_train = ...
    ...
    with torch.no_grad():
        ...
        # TODO: 用自赋值方式把 x_valid、y_valid 迁移到 device
        x_valid = ...
        y_valid = ...
        ...
        # TODO: 计算 y_valid_pred = w * x_valid + b
        y_valid_pred = ...

        # 验证集 loss：与训练 loss 同口径，便于对比训练/验证差距、监控过拟合
        # loss_valid 为标量，形状 []
        # TODO: 调用 mse_loss，传入 y_valid_pred 和 y_valid
        loss_valid = ...

        # MAE：平均偏差，单位与 y 相同
        # mae_valid 为标量，形状 []
        # TODO: 调用 mae_metric，传入 y_valid_pred 和 y_valid
        mae_valid = ...

        # R²：越接近 1，模型解释能力越强
        # r2_valid 为标量，形状 []
        # TODO: 调用 r2_score，传入 y_valid_pred 和 y_valid
        r2_valid = ...
...
with torch.no_grad():
    ...
    # TODO: 用自赋值方式把 x_test、y_test 迁移到 device
    x_test = ...
    y_test = ...
    ...
    # y_test_pred 形状 [10, 1]
    # TODO: 计算 y_test_pred = w * x_test + b
    y_test_pred = ...
    ...
    # loss_test 为标量，形状 []
    # TODO: 调用 mse_loss，传入 y_test_pred 和 y_test，得到 loss_test
    loss_test = ...

    # MSE：与训练时的 loss 同口径，作为参照
    # mse_test 为标量，形状 []
    # TODO: 调用 mse_loss，传入 y_test_pred 和 y_test
    mse_test = ...

    # MAE：平均偏差，单位与 y 相同，更直观
    # mae_test 为标量，形状 []
    # TODO: 调用 mae_metric，传入 y_test_pred 和 y_test
    mae_test = ...

    # R²：越接近 1，说明模型对数据变异的解释能力越强
    # r2_test 为标量，形状 []
    # TODO: 调用 r2_score，传入 y_test_pred 和 y_test
    r2_test = ...
