...
model_sequential = nn.Sequential(nn.Linear(in_features=1, out_features=1))
...
# 验证集指标对象
mae_metric_valid = MeanAbsoluteError().to(device)
r2_metric_valid = R2Score().to(device)

# 测试集指标对象
mae_metric_test = MeanAbsoluteError().to(device)
r2_metric_test = R2Score().to(device)
...
for epoch in range(epochs):
    ...
    # loss_batch 为标量，形状 []
    loss_batch = loss_fn(y_batch_pred, y_batch_train)
    ...
    mae_metric_train.update(y_batch_pred.detach(), y_batch_train)
    r2_metric_train.update(y_batch_pred.detach(), y_batch_train)
    ...
    with torch.no_grad():
        ...
        # MAE：平均偏差，单位与 y 相同
        # mae_train 为标量，形状 []
        mae_train = mae_metric_train.compute()

        # R²：越接近 1，模型解释能力越强
        # r2_train 为标量，形状 []
        r2_train = r2_metric_train.compute()
    ...
    with torch.no_grad():
        ...
        # MAE：平均偏差，单位与 y 相同
        # mae_valid 为标量，形状 []
        mae_metric_valid.update(y_valid_pred, y_valid)
        mae_valid = mae_metric_valid.compute()
        mae_metric_valid.reset()

        # R²：越接近 1，模型解释能力越强
        # r2_valid 为标量，形状 []
        r2_metric_valid.update(y_valid_pred, y_valid)
        r2_valid = r2_metric_valid.compute()
        r2_metric_valid.reset()
...
with torch.no_grad():
    ...
    # mae_test 为标量，形状 []
    mae_metric_test.update(y_test_pred, y_test)
    mae_test = mae_metric_test.compute()

    # R²：越接近 1，说明模型对数据变异的解释能力越强
    # r2_test 为标量，形状 []
    r2_metric_test.update(y_test_pred, y_test)
    r2_test = r2_metric_test.compute()
